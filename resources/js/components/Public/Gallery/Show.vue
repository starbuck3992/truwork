<template>
    ...
    <router-link :to="{ name: 'galleriesIndex', query:{category: slug} }">
        <p class="mt-10 sm:text-lg sm:font-extrabold text-gray-900 ml-10 flex font-medium text-xs">
            <ArrowCircleLeftIcon class="w-8 h-8 sm:w-8 sm:h-8"></ArrowCircleLeftIcon>
            <span class="ml-2">Zpět na seznam galerií</span>
        </p>
    </router-link>
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
                Api.get(`/api/galleries/${route.params.slug}`).then(response => {
